...
fn extract_local_override_definitions<'db>(
    ...
    let mut emitted_functions = smallvec::SmallVec::<[FunctionType<'db>; 1]>::new();
    ...
    for definition in end_of_scope_function_definitions(db, subclass_scope, &member.name) {
        ...
        {
            if emitted_functions.contains(&function) {
                continue;
            }
            definitions.push(LocalOverrideDefinition::from_function_type(
            ...
            infer_definition_types(db, definition).function_type(definition)
        {
            ...
            definitions.push(LocalOverrideDefinition::from_definition(
                db,
                definition,
...
fn extract_member_functions_from_type<'db>(
    ...
    match ty {
        Type::PropertyInstance(property) => {
            ...
            // should only consider accessors defined by the member under analysis.
            for accessor in [
                property.getter(db),
                property.setter(db),
                property.deleter(db),
            ...
            .flatten()
            {
                ...
                functions.extend(accessor_functions.into_iter().filter(|function| {
                    is_local_member_function(db, *function, member_name, member_scope)
                }));
            }
        ...
        Type::Union(union) => {
            ...
            for member in union.elements(db) {
                functions.extend(extract_member_functions_from_type(
                    db,
                    *member,
                    member_name,
                    member_scope,
